def Hhrep(H, n, P):
    ...
    b = [0] * n
    for i in range(n + 1):
        # add an arbitrary constraint to only allow one solution, namely ai =
        # -1
        _A = A + [[0] * i + [1] + [0] * (n - i)]
...
def Shrep(A, n, S):
    ...
    if r < n + 1:
        ...
        G = frozenset( vsub( s , S[0] ) for s in S[1:] )
        H = ( (0,) * i + (1,) + (0,) * (n - i - 1) for i in range(n) )
        O = _maxindset(A, G, H)
        S = S + tuple( vadd( o , S[0] ) for o in O )
